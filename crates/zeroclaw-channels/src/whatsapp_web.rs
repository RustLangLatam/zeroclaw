...
impl WhatsAppWebChannel {
    ...
    fn is_number_allowed_for_list(allowed_numbers: &[String], phone: &str) -> bool {
        ...
        crate::allowlist::is_user_allowed_by(allowed_numbers, phone, Self::phone_entry_matches)
    }
    ...
    fn is_number_explicitly_allowed(&self, phone: &str) -> bool {
        (self.peer_resolver)()
            .iter()
            .any(|entry| Self::phone_entry_matches(entry, phone))
    }

    /// A `*` entry has no digits, so it never matches here.
    #[cfg(feature = "whatsapp-web")]
    fn phone_entry_matches(entry: &str, phone: &str) -> bool {
        match (
