...
tool-git-forge-error-pull-merge-method = pull.merge の 'method' は 'merge'、'squash'、または 'rebase' である必要があります。
tool-git-forge-error-review-verdict = review.create の 'verdict' は approve|request_changes|comment である必要がありますが、'{ $verdict }' が指定されました。
tool-git-forge-error-unknown-cell = 不明またはサポートされていないリソース/アクション '{ $resource }.{ $action }' です。サポートされているグリッドを確認するにはアクション 'describe' を呼び出すか、リストにないものには 'raw' を使用してください。
tool-git-forge-error-no-channels = 利用可能なチャンネルがまだありません（チャンネルが初期化されていません）。
tool-git-forge-error-raw-requires-method = 'raw' には 'method' が必要です。
tool-git-forge-error-raw-requires-path = 'raw' には 'path' が必要です。
tool-git-forge-error-requires-resource = 型付き呼び出しには 'resource' が必要です（またはアクション 'raw'/'describe' を使用してください）。
tool-git-forge-error-missing-repo = 'repo' がありません（'owner/repo' が必要です）。
...
tool-browser-screenshot-error-missing-filename = スクリーンショットのパス「{ $path }」にファイル名の構成要素がありません
tool-browser-screenshot-error-runtime-config-target = ランタイム設定パス「{ $target }」にはスクリーンショットを書き込めません
tool-browser-screenshot-error-symlink-target = シンボリックリンクのターゲット「{ $target }」にはスクリーンショットを書き込めません
tool-browser-screenshot-error-path-not-utf8 = スクリーンショットのパス「{ $path }」は UTF-8 以外のパス名に解決されます。情報損失を伴う変換による書き込みは拒否されます
tool-browser-screenshot-error-computeruse-non-string-path = スクリーンショットの「path」パラメーターは文字列である必要があります。指定された値: { $path }
tool-browser-screenshot-error-non-string-path = スクリーンショットの「path」は文字列または省略である必要があります
tool-browser-screenshot-error-args-not-object = スクリーンショットの引数は JSON オブジェクトである必要があります
tool-browser-screenshot-error-sidecar-no-png-data = computer-use サイドカーから PNG データが返されませんでした
...
tool-web-search-tool-error-duckduckgo-blocked = DuckDuckGo がこのマシンへのリクエストをレート制限しています。検索を再試行したり言い換えたりせず、数分待ってから web_fetch で既知の URL を直接取得するか、Web 検索プロバイダーとして SearXNG、Brave、または Tavily を設定してください。
tool-web-search-tool-error-searxng-not-configured = SearXNG インスタンス URL が設定されていません。config.toml の [web_search] searxng_instance_url を設定するか、ZEROCLAW_web_search__searxng_instance_url 環境変数で上書きしてください。
tool-web-search-tool-note-truncated-results = （以降の結果は省略）
