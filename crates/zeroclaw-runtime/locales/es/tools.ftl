...
tool-channel-room-error-approval-required = Este canal no crea salas ni invita usuarios sin que un operador apruebe cada llamada: agrega "channel_room" a always_ask en el perfil de riesgo del agente.
...
tool-git-forge-error-pull-merge-method = pull.merge 'method' debe ser 'merge', 'squash' o 'rebase'.
tool-git-forge-error-review-verdict = review.create 'verdict' debe ser approve|request_changes|comment, se obtuvo '{ $verdict }'.
tool-git-forge-error-unknown-cell = resource/action '{ $resource }.{ $action }' desconocido o no admitido. Llama a la acción 'describe' para ver la cuadrícula admitida, o usa 'raw' para cualquier cosa no listada.
tool-git-forge-error-no-channels = Aún no hay canales disponibles (los canales no están inicializados).
tool-git-forge-error-raw-requires-method = 'raw' requiere 'method'.
tool-git-forge-error-raw-requires-path = 'raw' requiere 'path'.
tool-git-forge-error-requires-resource = una llamada tipada requiere 'resource' (o usa la acción 'raw'/'describe').
tool-git-forge-error-missing-repo = Falta 'repo' (se esperaba 'owner/repo').
...
tool-browser-screenshot-error-missing-filename = A la ruta de captura de pantalla «{ $path }» le falta el componente del nombre de archivo
tool-browser-screenshot-error-runtime-config-target = No se puede escribir la captura de pantalla en la ruta de configuración del tiempo de ejecución «{ $target }»
tool-browser-screenshot-error-symlink-target = No se puede escribir la captura de pantalla en el destino del enlace simbólico «{ $target }»
tool-browser-screenshot-error-path-not-utf8 = La ruta de captura de pantalla «{ $path }» se resuelve en un nombre de ruta que no está en UTF-8; se rechaza la escritura mediante una conversión con pérdida
tool-browser-screenshot-error-computeruse-non-string-path = El parámetro «path» de la captura de pantalla debe ser una cadena, se obtuvo { $path }
tool-browser-screenshot-error-non-string-path = «path» de la captura de pantalla debe ser una cadena o estar ausente
tool-browser-screenshot-error-args-not-object = Los argumentos de captura de pantalla deben ser un objeto JSON
tool-browser-screenshot-error-sidecar-no-png-data = El sidecar computer-use no devolvió datos PNG
tool-browser-screenshot-error-sidecar-empty-png = El sidecar computer-use devolvió una carga útil de captura de pantalla vacía
tool-browser-screenshot-error-sidecar-not-png = El sidecar computer-use devolvió una carga útil de captura de pantalla que no es PNG
tool-browser-screenshot-error-sidecar-non-json-success = El sidecar computer-use devolvió una respuesta correcta que no es JSON para una captura de pantalla con ruta; no se escribió el archivo solicitado
...
tool-web-search-tool-error-duckduckgo-blocked = DuckDuckGo está limitando la frecuencia de solicitudes de esta máquina. No vuelvas a intentar ni reformules la búsqueda; espera unos minutos, obtén directamente las URL conocidas con web_fetch o configura SearXNG, Brave o Tavily como proveedor de web_search.
tool-web-search-tool-error-searxng-not-configured = La URL de la instancia de SearXNG no está configurada. Establece [web_search] searxng_instance_url en config.toml o reemplázala mediante la variable de entorno ZEROCLAW_web_search__searxng_instance_url.
tool-web-search-tool-note-truncated-results = (se omitieron más resultados)
